...
export function stringsToIntegers(numbers: string[]): number[] {
    const convert = numbers.map((num) => (parseInt(num) ? parseInt(num) : 0));
    return convert;
}
...
export function countShortWords(words: string[]): number {
    const allShortWords = words.filter((word: string) => word.length < 4);
    return allShortWords.length;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    if (negIndex < 0) {
        ...
        const listBeforeNeg = values.slice(0, negIndex);
        const sum = listBeforeNeg.reduce((acc, curr) => acc + curr, 0);
